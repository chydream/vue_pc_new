          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button
                ...
                v-hasPermission="permission.edit">编辑</el-button>
              <el-button
                type="text"
                size="small"
                @click="handleDelete(scope.row)"
                v-hasPermission="permission.delete">删除</el-button>
            </template>
          </el-table-column>
        </cvue-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleList(keyword, status) {
      ...
      this.$store
        .dispatch("demo/GetDemoList", params)
        .then((res) => {
          // console.log(res)
          if (res.success) {
            // var data = res.data[0]
    ...
    handleDelete(row) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        .then(() => {
          // console.log(row.id)
          this.$store
            .dispatch("home/DeleteCybercafesDetail", row.id)
            .then((res) => {
              // console.log(res)
              if (res.code == 1) {
                this.$message({
...
.home {
  ...
  .el-table__body {
    ...
    tr td:nth-child(8) span {
      // color: #409EFF;
      cursor: pointer;
    }
    tr td:nth-child(9) span,
    tr td:nth-child(10) span,
    tr td:nth-child(11) span,
    tr td:nth-child(12) span,
